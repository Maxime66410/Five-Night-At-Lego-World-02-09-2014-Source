...
screenLocalRotationX = cameraHolder.transform.rotation.x;
        ...
        {  if (hit.transform.name == "ScreenButton" && GetComponent(PowerUsage).battery != 0 )
	       ...
	       if (hit.transform.name == "DoorButtonL" && GetComponent(PowerUsage).battery != 0 || hit.transform.name == "doorLtouch" && GetComponent(PowerUsage).battery != 0)
	        { 
				...
				if(bear.GetComponent(FreddyBearAI).whoToMove.transform.position == bear.GetComponent(FreddyBearAI).spawnPointFive.transform.position && !doorLbutton.GetComponent(DoorButton).opened)
				{
	...
	if(doorLcounter==0 && !doorLbutton.GetComponent(DoorButton).opened) { doorLbutton.GetComponent(DoorButton).opened=true;}
	if(doorLcounter==1 && doorLbutton.GetComponent(DoorButton).opened) { doorLbutton.GetComponent(DoorButton).opened=false;}
	
	if(doorRcounter==0 && !doorRbutton.GetComponent(DoorButton).opened) { doorRbutton.GetComponent(DoorButton).opened=true;}
	if(doorRcounter==1 && doorRbutton.GetComponent(DoorButton).opened) { doorRbutton.GetComponent(DoorButton).opened=false;}
	if(GetComponent(PowerUsage).battery == 0) {
	
	bear.GetComponent(FreddyBearAI).weAreGood=false;
	doorRbutton.GetComponent(DoorButton).opened = false;
